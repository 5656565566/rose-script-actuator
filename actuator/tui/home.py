...
class HomeScreen(PageScreen):
    ...
    def compose(self) -> ComposeResult:
        with Content():
            ...
            with Collapsible(title="示范"):
                yield Markdown(EXAMPLE_MD)
            with Collapsible(title="如何使用?"):
                yield Markdown(ABOUT_MD)
            with Collapsible(title="基础函数 API"):
                yield Markdown(API_MD_1)
            with Collapsible(title="协程库 API"):
                yield Markdown(API_MD_2)
            with Collapsible(title="字符串库 API"):
                yield Markdown(API_MD_3)
            with Collapsible(title="表库 API"):
                yield Markdown(API_MD_4)
            with Collapsible(title="数学库 API"):
                yield Markdown(API_MD_5)
            with Collapsible(title="文件操作库 API"):
                yield Markdown(API_MD_6)
            with Collapsible(title="操作系统库 API"):
                yield Markdown(API_MD_7)
            with Collapsible(title="调试库 API"):
                yield Markdown(API_MD_8)
            with Collapsible(title="UTF-8 库 API"):
                yield Markdown(API_MD_9)
            with Collapsible(title="包管理 API"):
                yield Markdown(API_MD_10)
